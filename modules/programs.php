...
final class WPSG_Programs extends WPSG_PostContent_Modules {
	...
	protected function __construct() {
        ...
        add_action( 'admin_post_wpsg_save_program', 'wpsg_handle_save_program' );
	}
    ...
    /**
     * Load program data for edit mode based on request
     */
	...
	public function render_list() {
        ...
        $view_mode = $_GET['view'] ?? 'programs';

        $programs = $this->service->get_list('program', ['order'=>'published_at']);
        ...
        <div class="wrap wpsg-view-<?php echo esc_attr( $view_mode ); ?>">
            ...
            <?php foreach ( $programs as $item ): ?>
                ...
                <?php $program = $this->get_data( $item->id ); 

                $main_data = (array) $program['main'];
                $meta_data = $program['meta'];
                ?>
				<div class="wpsg-program-item">
                    <div class="wpsg wpsg-boxed">
                        <div class="wpsg-row-full">
                            ...
                            <div class="wpsg-row">
                                <div class="col-2">
                                    <img src="<?php echo $meta_data['image'] ?? WPSG_URL . 'assets/images/placeholder.png'; ?>" alt="<?php echo esc_attr( $main_data['title'] ?? 'Program Image' ); ?>" style="max-width:100%; height:auto;" />
                                </div>
                                <div class="col-10">
                                    <div class="wpsg-row">
                                        <div class="wpsg-row-full">
    ...
    /**
     * Render the program form for add/edit
     */
	public function render_form() {
		...
		<form method="post" action="<?php echo $action; ?>" class="form">
									...
									<div class="col-4">
										...
										<div class="wpsg-form-field">
											...
											<div id="program_image_wrapper">
												...
												<?php endif; ?>
												<input type="hidden" id="program_image" name="program[image]" value="<?php echo esc_attr($this->data['image'] ?? ''); ?>" />
											</div>
            ...
            <script type="text/javascript" data-wpsg="program-image-handler">
            jQuery(function ($) {

                let frame;

                $('#program_image_select').on('click', function (e) {
                    e.preventDefault();
                    ...
                    frame.on('select', function () {
                        ...
                        $('#program_image_wrapper img').remove();
                        $('#program_image_wrapper').prepend(img);
                    });

                    frame.open();
                });

                $('#program_image_remove').on('click', function (e) {
                    e.preventDefault();
    ...
    public function wpsg_handle_save_program() {
        ...
        if (
            ...
            ! wp_verify_nonce( $_POST['wpsg_nonce'], 'wpsg_save_program' )
        ) {
        ...
        if ( $title === '' ) {
            wp_die( 'Program title is required.' );
        }
        ...
        if ( $id > 0 ) {
            ...
            // CREATE
            $id = $service->create( 'program', $init_data );
        ...
        if ( $id ) {
            ...
            if ( isset( $program['description'] ) ) {
                $service->set_meta(
                    $id,
                    'description',
                    wp_kses_post( $program['description'] )
                );
            }
