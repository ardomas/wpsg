<?php
/**
 * modules/frontend/reports/main.php
 */

if( !fe_check_default_requirement() ){
    wp_redirect('/app');
}

$url_params = $_GET;
$new_params = [];

$sid = $_GET['sid'];

$user = wp_get_current_user();
$json_file = __DIR__ . '/assets/json/data.json';
$json_data = wpsg_get_json_data( $json_file );

$json_menu   = $json_data['menu'  ] ?? [];
$json_scores = fe_get_app_json_score_data();

$menu = [];
foreach( $json_menu as $key=>$item ){
    $sub = wpsg_encode_keys( [$user->ID, $key] );
    // $item['url'] = fe_get_app_url() . '/settings?sid=' . $sid . '&s1=' . $sub;
    $menu[$sub] = $item;
}

if( !isset( $_GET['s1'] ) ){

    echo fe_render_href_menu_text_list([ 'menu'=>$menu, 'key_id_name'=>'s1', 'url_params' => ['sid'=>$sid] ]);

} else {

    $subkey = $_GET['s1'];
    $sub_menu = $menu[$subkey] ?? null;

    $path = $sub_menu['path'];
    $file = $sub_menu['file'];

    $str_file = WPSG_DIR . '/modules/' . $path . '/' . $file;
    print_r( $str_file );

    if( file_exists( $str_file ) ){

        require $str_file;

    } else {
        $base_filename = basename( $str_file );
        ?><div class="alert alert-danger py-5">
            Problem:
            <ul class="text-left mb-3">
                <li>Path: <code><strong><?php echo esc_html( $path ); ?></strong></code></li>
                <li>File name: <code><strong><?php echo esc_html( $str_file ); ?></strong></code></li>
                <li>Status: File not found!</li>
            </ul>
            Solution:
            <p class="mb-3">Check the file path or contact the administrator.</p>
        </div><?php

    }
}
